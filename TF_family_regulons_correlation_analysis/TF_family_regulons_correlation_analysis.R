# This script performs an analysis of correlation between expression pattern of genes of a certain group and presence (or absence)
# of binding sites of a particular TF families within the promoters of these genes. Bar plots are generated as the output.
# In order to produce plots of adequate size, I indcluded the '+TF/-TF' ranking in this script.
...
# Load the annotated MAST output (only columns 'GeneID', 'TF_name', 'TF_family')
tf_analysis_input_annotated <- read.csv('../MAST_XML_parser/tf_analysis_input_annotated.tsv', sep = '\t')[, c(1, 5, 6)]

# Load the input dataframe containing gene groups of interest. Groups are denoted in the "Gene_group" column
input_table <- read.csv('Genes_of_interest.tsv', sep = '\t')

input_table$GeneID <- as.character(input_table$GeneID)
tf_analysis_input_annotated$GeneID <- as.character(tf_analysis_input_annotated$GeneID)

input_table <- merge(input_table, tf_analysis_input_annotated, by = 'GeneID', all.x = T)

# Processing the input dataframe: this need to count the number of genes in the groups
groups <- unique(input_table$Gene_group)
...
no_duplications <- rownames(unique(input_table[, c("GeneID", "TF_family", 'Gene_group')]))
...
counts_genes <- lapply(groups, function(x)
  data.frame(TF_family = unique(input_table[input_table$Gene_group == x, 'TF_family'])))
...
for (i in 1:length(groups)) {
  ...
  counts_genes[[i]]$Up_regulated <- sapply(t, function(x) sum(input_table[input_table$TF_family == x & input_table$Gene_group == groups[i], 'Up_regulated'], na.rm = T))
  counts_genes[[i]]$Down_regulated <- sapply(t, function(x) sum(input_table[input_table$TF_family == x & input_table$Gene_group == groups[i], 'Down_regulated'], na.rm = T))
...
chosen_group <- input_table[input_table$Gene_group == chosen_group_name, ]
...
for (i in 1:length(tf_families_names)){
  ...
  sites_presence <- lapply(c(1, 2), function(x) data.frame(t(colSums(sites_presence[[x]], na.rm = T))))
...
# Create the "+TF/-TF" plot (one with coloured bars)
barplot_top_ratios <- 
...
ggsave(png_path,  output, width = 18, height = 6, units = 'cm', dpi = 600, limitsize = F)

# Notify when the barplots done
